...
void Compiler::enterStored_definition(ModelicaParser::Stored_definitionContext *ctx) {
  ...
  auto mem = std::make_shared<ast::Modelica>("1.0");
  auto node = mem.get();
...
void Compiler::enterStored_definition_class(ModelicaParser::Stored_definition_classContext *ctx) {
  ...
  parent->classDefinition(ast::ClassDefinition(""));

  // node
  auto node = &(parent->classDefinition().get());
  node->final(ctx->FINAL() != nullptr);

  // no need to call setMemory since
  // all of the data is managed via the parent scope
...
void Compiler::enterClass_definition(ModelicaParser::Class_definitionContext *ctx) {
  ...
  parent->class_(ast::ClassContents(class_type));

  // node
  auto node = &(parent->class_().get());
...
void Compiler::exitClass_spec_comp(ModelicaParser::Class_spec_compContext *ctx) {
  ...
  // class definition
  auto cdef = getXml<ast::ClassDefinition>(ctx->parent->parent);
  cdef->comment(ctx->string_comment()->getText());
  ...
  // store ast annotation
  linkData(ctx, ctx->parent->parent);
}
...
void Compiler::exitDeclaration(ModelicaParser::DeclarationContext *ctx) {
  ...
  auto mem = std::make_shared<ast::String>(ctx->IDENT()->toString());
  auto node = mem.get();
...
void Compiler::exitEquation_section(ModelicaParser::Equation_sectionContext *ctx) {
  auto mem = std::make_shared<ast::EquationSection>();
  auto node = mem.get();
...
void Compiler::enterPrimary_unsigned_number(ModelicaParser::Primary_unsigned_numberContext *ctx) {
  auto mem = std::make_shared<ast::String>(ctx->toString());
  auto node = mem.get();
...
void Compiler::enterName(ModelicaParser::NameContext *ctx) {
  auto mem = std::make_shared<ast::Name>(ctx->toString());
  auto node = mem.get();
