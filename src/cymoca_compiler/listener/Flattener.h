 ...
 public:
  ...
  void exit(Class &ctx) override {
    ...
    for (auto &iter: dict.memory()) {
      ...
      // handle predefined types, see Spec 4.8 Predefined Types and Classes
      if (comp.type().name() == "Real") {
        ...
        pairsToAdd.push_back(
            {name + ".quantity", make_unique<Component>(name + ".quantity", "StringType", Prefix::PARAMETER)});
        // parameter StringType unit = "" "Unit used in equations";
        pairsToAdd.push_back(
            {name + ".unit", make_unique<Component>(name + ".unit", "StringType", Prefix::PARAMETER)});
        // parameter StringType displayUnit = "" "Default display unit";
        pairsToAdd.push_back(
            {name + ".displayUnit", make_unique<Component>(name + ".displayUnit", "StringType", Prefix::PARAMETER)});
        // parameter RealType min=-Inf, max=+Inf; // Inf denotes a large value
        pairsToAdd.push_back(
            {name + ".min", make_unique<Component>(name + ".min", "RealType", Prefix::PARAMETER)});
        // parameter RealType start = 0; // Initial value
        pairsToAdd.push_back(
            {name + ".max", make_unique<Component>(name + ".max", "RealType", Prefix::PARAMETER)});
        // parameter RealType start = 0; // Initial value
        pairsToAdd.push_back(
            {name + ".start", make_unique<Component>(name + ".start", "RealType", Prefix::PARAMETER)});
        // parameter BooleanType fixed = true, // default for parameter/constant;
        // = false; // default for other variables
        pairsToAdd.push_back(
            {name + ".fixed", make_unique<Component>(name + ".fixed", "BooleanType", Prefix::PARAMETER)});
        // parameter RealType nominal; // Nominal value
        pairsToAdd.push_back(
            {name + ".nominal", make_unique<Component>(name + ".nominal", "RealType", Prefix::PARAMETER)});
        // parameter BooleanType unbounded=false; // For error control
        pairsToAdd.push_back(
            {name + ".unbounded", make_unique<Component>(name + ".unbounded", "BooleanType", Prefix::PARAMETER)});
        // parameter StateSelect stateSelect = StateSelect.default;
        pairsToAdd.push_back(
            {name + ".stateSelect", make_unique<Component>(name + ".stateSelect", "StateSelect", Prefix::PARAMETER)});
        // TODO assert(value >= min and value <= max, "Variable value out of limit");
        ...
        pairsToAdd.push_back(
            {name + ".min", make_unique<Component>(name + ".min", "IntegerType", Prefix::PARAMETER)});
        pairsToAdd.push_back(
            {name + ".max", make_unique<Component>(name + ".max", "IntegerType", Prefix::PARAMETER)});
        // parameter IntegerType start = 0; // Initial value
        pairsToAdd.push_back(
            {name + ".start", make_unique<Component>(name + ".start", "IntegerType", Prefix::PARAMETER)});
        // parameter BooleanType fixed = true, // default for parameter/constant;
        // = false; // default for other variables
        pairsToAdd.push_back(
            {name + ".fixed", make_unique<Component>(name + ".fixed", "BooleanType", Prefix::PARAMETER)});
        // TODO equation assert(value >= min and value <= max, "Variable value out of limit");
        ...
        pairsToAdd.push_back(
            {name + ".start", make_unique<Component>(name + ".start", "BooleanType", Prefix::PARAMETER)});
        // parameter BooleanType fixed = true, // default for parameter/constant;
        // = false, // default for other variables
        pairsToAdd.push_back(
            {name + ".fixed", make_unique<Component>(name + ".fixed", "BooleanType", Prefix::PARAMETER)});
        ...
        pairsToAdd.push_back(
            {name + ".start", make_unique<Component>(name + ".start", "StringType", Prefix::PARAMETER)});
        // parameter BooleanType fixed = true, // default for parameter/constant;
        // = false, // default for other variables
        pairsToAdd.push_back(
            {name + ".fixed", make_unique<Component>(name + ".fixed", "BooleanType", Prefix::PARAMETER)});
        ...
        pairsToAdd.push_back(
            {name + ".min", make_unique<Component>(name + ".min", "EnumType", Prefix::PARAMETER)});
        pairsToAdd.push_back(
            {name + ".max", make_unique<Component>(name + ".max", "EnumType", Prefix::PARAMETER)});
        // parameter EnumType start = e1; // Initial value
        pairsToAdd.push_back(
            {name + ".start", make_unique<Component>(name + ".start", "EnumType", Prefix::PARAMETER)});
        // parameter BooleanType fixed = true, // default for parameter/constant;
        // = false; // default for other variables
        pairsToAdd.push_back(
            {name + ".fixed", make_unique<Component>(name + ".fixed", "BooleanType", Prefix::PARAMETER)});
    ...
    for (auto &keyval: pairsToAdd) {
      keyval.second->parent(&ctx.components());
      ctx.components().memory()[keyval.first] = move(keyval.second);
    }
    _stack.pop();
  }
 protected:
